...
fs.readFile("./input.txt", { encoding: "utf8" }, (err: any, data: string) => {
  ...
  function part1() {
    ...
    function updateCube(x: number, y: number, z: number): string {
  ...
  function part2() {
    interface Map {
      [x: number]: { [y: number]: { [z: number]: { [w: number]: string } } };
    }
    const map: Map = {};
    for (let x = 0; x < input.length; x++) {
      map[x] = {};
    ...
    function updateCube(x: number, y: number, z: number, w: number): string {
      let counter: number = 0;
      for (let i = x - 1; i < x + 2; i++) {
        for (let j = y - 1; j < y + 2; j++) {
          for (let m = z - 1; m < z + 2; m++) {
